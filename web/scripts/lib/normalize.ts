...
export function extractCity(addr: string, pref: Pref): string | null {
  ...
  if (m) return m[1];
  const re2 = new RegExp(`${pref}(さいたま市[^0-9\\-\\s]+?区|横浜市[^0-9\\-\\s]+?区|川崎市[^0-9\\-\\s]+?区|千葉市[^0-9\\-\\s]+?区|相模原市[^0-9\\-\\s]+?区|名古屋市[^0-9\\-\\s]+?区|大阪市[^0-9\\-\\s]+?区|堺市[^0-9\\-\\s]+?区|京都市[^0-9\\-\\s]+?区)`);
...
const GENRE_KEYWORDS: Array<[string, string[]]> = [
  ...
  ["焼肉", ["焼肉", "焼き肉", "ヤキニク", "ホルモン", "ホルモン焼"]],
  ["寿司", ["寿司", "鮨", "回転寿司", "にぎり寿司", "寿し"]],
  ["ラーメン", ["ラーメン", "らーめん", "中華そば", "つけ麺", "担々麺", "まぜそば"]],
  ["うどん", ["うどん", "讃岐うどん"]],
  ["そば", ["蕎麦", "十割そば", "手打ちそば"]],
  ["パスタ", ["パスタ", "スパゲッティ", "スパゲティ", "ナポリタン", "ペペロンチーノ"]],
  ["ピザ", ["ピザ", "ピッツァ", "Pizza", "Pizzeria"]],
  ["イタリアン", ["イタリアン", "Italian", "Osteria", "オステリア", "トラットリア", "Ristorante"]],
  ["フレンチ", ["フレンチ", "French", "ビストロ", "Bistro", "Brasserie"]],
  ["中華", ["中華料理", "餃子", "四川料理", "台湾料理", "北京料理", "広東料理", "上海料理", "麻婆"]],
  ["韓国料理", ["韓国料理", "サムギョプサル", "チーズタッカルビ", "韓国チキン"]],
  ["タイ料理", ["タイ料理", "ガパオ", "トムヤム"]],
  ["ベトナム料理", ["ベトナム料理", "生春巻"]],
  ...
  ["カフェ", ["カフェ", "Cafe", "CAFE", "café", "喫茶店", "珈琲店", "コーヒースタンド", "コーヒー専門"]],
  ["ベーカリー", ["ベーカリー", "Boulangerie", "ブーランジェリー", "パン屋", "食パン専門", "高級食パン", "デニッシュ専門"]],
  ["スイーツ", ["スイーツ", "ケーキ屋", "パティスリー", "ジェラート", "クレープ", "ドーナツ", "タピオカ", "かき氷", "マカロン", "アイスクリーム"]],
  ["和菓子", ["和菓子", "大福", "団子", "最中", "どら焼き"]],
  ["定食", ["定食屋", "食堂", "おふくろの味"]],
  ["鉄板焼", ["鉄板焼", "お好み焼", "もんじゃ", "たこ焼"]],
  ["うなぎ", ["うなぎ", "鰻"]],
  ["天ぷら", ["天ぷら専門", "天麩羅"]],
  ["餃子", ["餃子専門", "焼き餃子専門", "餃子専門店"]],
  ["海鮮", ["海鮮料理", "魚介料理", "魚料理専門"]],
];

export function extractGenre(text: string, title?: string): string | null {
  if (title) {
    for (const [genre, kws] of GENRE_KEYWORDS) {
      for (const kw of kws) {
        if (title.includes(kw)) return genre;
      }
    }
  }
